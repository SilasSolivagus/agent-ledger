...
**两家各自长什么样**——同时装了两个 agent 的时候，首页会多一张并排画像。**它不是排名**，原因写在下一节。
...
- **没有按项目配对的对比**。真要拿两家比同一件事，只能挑两边都干过的项目（本机是 `shr`、`Scrapling` 这几个）分别看，现在还得手动挑。
- **没有折算成钱**。要算「若按 API 价目付费值多少」需要一张你自己维护的模型价目表，还没做。
...
```sh
pnpm install && pnpm run check   # 构建 + 31 个测试
```
