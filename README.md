...
编码 agent 的运行记录：**实时轨迹**、**逐条账本**、**真实花费**。支持 Claude Code、Codex、Cursor、WorkBuddy。

这些 agent 都会把每次会话完整写在本机，但都没有提供查看的地方，只能在终端里向上翻。本工具读取这些记录，还原为完整轨迹和一张总账。
...
**列会话不读文件，只有你点开的那个才被解析**，解析完按文件修改时间缓存——所以一千多个会话也开得动，正在写入的那个会自动重读。

**没有硬上限。** 服务启动后在后台逐文件统计，总量和会话列表都覆盖整个窗口，结果按文件的修改时间与大小缓存在 `~/.runledger/tally.json`，重启不重算。

统计期间侧栏显示「正在统计 466 / 1912 · 约剩 28 秒」，剩余时间按本次实测速率外推。**统计属于服务而非请求**：切换 tab 不会重启、不会排队、也不会丢弃已算的部分；每 200 个文件落一次盘，中途关闭只损失最后一小段。

完成后总量标「全窗口」，可直接与厂商用量页对账（按自然日划分）。`--limit` 退化为统计完成前的兜底。
...
- **价目表是手抄的**。88 个型号，覆盖这四家 agent 会调用的 Claude / GPT / o 系列；跟着版本更新，`npm run prices:check` 会告诉你上游动了没，但它不会自己改。非美元的条目（DeepSeek 按元计价）上游验不了，得手工核。用了表外型号的记录会显示「无此型号价格」，不会显示成 0。
- **没有筛选和搜索**。会话列表按时间排，一千多个会话里找特定的那个只能靠肉眼。
...
```sh
pnpm install && pnpm run check          # 构建 + 127 个测试
